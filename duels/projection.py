...
gm = Gamemode.DUELS
def calculate_projection(statsdata, gametype, currentdata, wins=None, wlr=None, days=None):
    gametype = get_duel_type(gametype)
    dailywins, dailylosses, dailygoals, gametimeagame = statsdata.get_stats(gm, gametype=gametype)  # @UnusedVariable
    bridgedata = currentdata.pop(gm)
    currentwins, currentlosses = bridgedata[f"{gametype}_wins"], bridgedata[f"{gametype}_losses"]
    ...
    return {**overall[max(overall.keys())], **currentdata, **extra, "gamemode": gm, "gametype": gametype}
...
def wrap_extra_data(dailywins, dailylosses, gametimeagame, mostmap, accuracy): 
    ...
    return {"dailywins" : round(dailywins, 1), "dailylosses" : round(dailylosses, 1), 
            ...
            "dailywlr" : dailywlr, "mostmap" : mostmap, "accuracy" : str(accuracy) + "%", "accuracynum": accuracy}
